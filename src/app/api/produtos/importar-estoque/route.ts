...
export async function POST(request: Request) {
  ...
  if (fetchError) {
    return NextResponse.json({ error: "Não foi possível ler o catálogo atual." }, { status: 500 });
  }
  ...
  for (const grupo of chunk(paraAtualizar, CHUNK_SIZE)) {
    ...
    if (error) erros += grupo.length;
    else atualizados += grupo.length;
  }
  ...
  return NextResponse.json({ total: linhas.length, atualizados, criados, erros, ignoradas });
}
