  ...
  return (
    <Card className="flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-sm font-semibold text-gray-900">{pedido.clientes?.nome ?? "Cliente"}</p>
          <p className="text-xs text-gray-400">{formatRelativeTime(pedido.criado_em)}</p>
        </div>
      ...
      <ul className="space-y-1 rounded-lg bg-gray-50 p-2.5 text-xs text-gray-600">
        {pedido.itens_pedido.length === 0 && <li className="text-gray-400">Sem itens</li>}
        {pedido.itens_pedido.map((item) => (
          <li key={item.id} className="flex justify-between gap-2">
            <span className="truncate">
              {item.quantidade}x {item.produtos?.nome ?? "Produto"}
            </span>
            <span className="shrink-0 text-gray-400">{formatCurrency(item.quantidade * Number(item.preco_unitario))}</span>
          </li>
        ))}
      </ul>
      ...
      <div className="flex items-center justify-between border-t border-gray-100 pt-3">
        <span className="text-sm font-semibold text-gray-900">{formatCurrency(Number(pedido.total ?? 0))}</span>
        {next && (
